# A Narcissistic Number is a number which is the sum of its own digits, each
# raised to the power of the number of digits.
# e.g. 153 (3 digits): 1^3 + 5^3 + 3^3 = 1 + 125 + 27 = 153
# e.g. 1634 (4 digits): 1^4 + 6^4 + 3^4 + 4^4 = 1 + 1296 + 81 + 256 = 1634
# Your code must return true or false depending upon whether the given number
# is a Narcissistic number. Error checking for text strings or other invalid
# inputs is not required, only valid integers will be passed into the function.

def narcissistic?(value)
  value.to_s.split("").map { |x| x.to_i**value.to_s.length }.reduce(:+) == value
end

narcissistic?(153)
narcissistic?(1634)
